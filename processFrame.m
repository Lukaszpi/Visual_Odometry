...
% tracked
S1.kp = new_kp(:, scores > 0.998 );
S1.corr = S0.corr( scores > 0.998 ); % array that stores indices of corresponding S1.p3D
...
%     S1.kp_cand  = fliplr( points( scores > 0.995, : ) )';
%     % only keep the transformation matrix and initial keypoint if the kp
%     % could be tracked
%     S1.T_cand   = S0.T_cand(:, scores > 0.995  );
%     S1.kp_cand0 = S0.kp_cand0(:, scores > 0.995 );
%     
...
% 
% % get rid of keypoints corresponding to landmarks
% keypoints = keypoints(:, sum( ismember( keypoints, ...
%                              round( S1.kp(1:2, :)) ), 1) ~= 2 );
% 
% % ad them to candidate list as well as initial list
% S1.kp_cand0 = [S0.kp_cand0 keypoints];
% S1.kp_cand = [S0.kp_cand keypoints];
% S1.T_cand = [S0.T_cand repmat(T(:), 1, size( keypoints, 2))];


%%% NEW TRY %%%
%%% Step 3 %%%
% Initialization of new candidate keypoints, same code as in initialization
...
%% These are the new keypoints, will be added to S1.kp_cand in the end
new_candidate_keypoints = selectKeypoints(harris_scores, num_keypoints, ...
                            nonmaximum_supression_radius);

first_observed_T = repmat(T(:), 1, size( new_candidate_keypoints, 2));

if ~isempty( S0.kp_cand )
    pointTracker = vision.PointTracker('BlockSize', [11 11]); 
    initialize(pointTracker, fliplr(S0.kp_cand(1:2, :)'),  img0);
    [points,tracked,score] = step(pointTracker, img1);
    S1.kp_cand  = fliplr( round(points( tracked, : )) )';
    S1.T_cand   = S0.T_cand(:, tracked  );
    S1.kp_cand0 = S0.kp_cand0(:, tracked );


    %% check triangulability COPIED FROM DOMINIK FOR NOW
    % perpendicular to camera plane
    min_baseline = 0.5;
    baseline = sum( (T(:, 4) - S1.T_cand(13:16, :)).^2, 1);
    ...
    readyToTr = baseline > min_baseline^2 & ...
                sum( rot_vec_diff.^2, 1) < 2;
    %% Triangulate, taken from initialization.m, does not work yet!!!
    
        %% apply RANSAC with 8-point algorithm
    keypoints_1 = [S1.kp_cand0(:, readyToTr ); ones(1, sum( readyToTr ) )];
    keypoints_2 = [S0.kp_cand(:, readyToTr ); ones(1, sum( readyToTr ) )];
    num_keypoints = length(keypoints_1);
